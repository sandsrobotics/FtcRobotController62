...
public class RobotHardware {
    ...
    public void initDriveMotors() {
        ...
        for (DcMotor motor : driveMotors) {
            if (hardwareSettings.flipDriveMotorDir[i])
    ...
    public void initLauncherMotors() {
        ...
        launcherWheelMotor.setPIDFCoefficients(DcMotor.RunMode.RUN_USING_ENCODER,
                HardwareSettings.launcherWheelMotorPID);
        ...
        // launcherIntakeServo = robot.hardwareMap.crservo.get("servo" +
        // hardwareSettings.launcherIntakeServoNum);
        ...
        if (hardwareSettings.flipLauncherMotorDir[0])
            launcherWheelMotor.setDirection(DcMotor.Direction.REVERSE);
        if (hardwareSettings.flipLauncherMotorDir[1])
            launcherIntakeMotor.setDirection(DcMotor.Direction.REVERSE);
        if (hardwareSettings.flipLauncherMotorDir[2])
            launcherLaunchServo.setDirection(Servo.Direction.REVERSE);
        if (hardwareSettings.flipLauncherMotorDir[3])
            launcherGateServo.setDirection(Servo.Direction.REVERSE);
        if (hardwareSettings.flipLauncherMotorDir[4])
            launcherFrogArmLeft.setDirection(Servo.Direction.REVERSE);
        if (hardwareSettings.flipLauncherMotorDir[5])
            launcherFrogArmRight.setDirection(Servo.Direction.REVERSE);
        // if(hardwareSettings.flipLauncherMotorDir[3])
        // launcherIntakeServo.setDirection(DcMotorSimple.Direction.REVERSE);
    ...
    public void initGrabberHardware() {
        ...
        if (hardwareSettings.flipGrabberMotorDir[0])
            grabberLifterMotor.setDirection(DcMotorSimple.Direction.REVERSE);
        if (hardwareSettings.flipGrabberMotorDir[1])
            grabberServos.get(0).setDirection(CRServo.Direction.REVERSE);
        if (hardwareSettings.flipGrabberMotorDir[2])
            grabberServos.get(1).setDirection(CRServo.Direction.REVERSE);
        ...
        grabberIntakeLimitSwitch = robot.hardwareMap.get(DigitalChannel.class,
                hardwareSettings.grabberIntakeLimitSwitchName);
    ...
    public void initVisionHardware() {
        ...
        if (hardwareSettings.flipMainCamServoDir)
            mainCamServo.setDirection(Servo.Direction.REVERSE);
    }
    ...
    public void initUltrasonicSensors() {
        ...
        // distSensorX2 = robot. hardwareMap.get(DFR304Range.class, "distSensor" +
        // hardwareSettings.X2UltrasonicNum);
        ...
        for (DFR304Range distSen : distSensors) {
            distSen.initialize(parameters);
        }
    }

    public void initMotorSettings(List<DcMotorEx> motors, DcMotor.ZeroPowerBehavior zeroPowerBehavior) {
        for (DcMotorEx motor : motors)
            initMotorSettings(motor, zeroPowerBehavior);
    }
    ...
    // set motor modes//
    ///////////////////
    public void setMotorsZeroPowerBehaviorList(List<DcMotorEx> motors, DcMotor.ZeroPowerBehavior zeroPowerBehavior) {
        for (DcMotorEx motor : motors)
            motor.setZeroPowerBehavior(zeroPowerBehavior);
    }

    public void setMotorsRunModeList(List<DcMotorEx> motors, DcMotor.RunMode runMode) {
        for (DcMotorEx motor : motors)
    ...
    public float getDistance(DFR304Range distSensor) {
        if (distSensor.getParameters().measureMode == DFR304Range.MeasureMode.PASSIVE) {
    ...
    public boolean motorPositionsInToleranceList(List<DcMotorEx> motors, int tolerance) {
        for (DcMotorEx m : motors) {
            if (Math.abs(m.getTargetPosition() - m.getCurrentPosition()) > tolerance)
                return false;
        }
        return true;
    }
...
class HardwareSettings {
    ...
    // launcher motors
    protected boolean[] flipLauncherMotorDir = { true, true, false, false, false, true };
